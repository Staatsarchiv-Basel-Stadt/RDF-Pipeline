...
describe('validateDate', () => {
  ...
  for (const input of inputs) {
    expect(() => date.validateDate(...input, false)).to.throw("This date does not exist");
  }
}),
it('should produce no error for valid dates', () => {
  ...
  for (const input of inputs) {
    expect(() => date.validateDate(...input, false)).to.not.throw("This date does not exist");
  }
})
})
...
describe("archScopeToEtdfDate", () => {
  it('should transform ArchScope format to EDTF format', () => {
    ...
    for (const example of examples) {
      assert.strictEqual(date.archScopeToEtdfDate(example.input), example.output)
    }
  }),
  it("should produce an error when date format is invalid", () => {
    const inputs = ["12-11-1989", "11.13.1568", "01011990", "-1990-01-03"]
    for (const input of inputs){
      expect(() => date.archScopeToEtdfDate(input)).to.throw();
    }
  })
})
...
describe("etdfToIsoDate", () => {
  it('should transform etdf date to ISO datetime (@ day beginning)', () => {
    ...
    for (const example of examples) {
      assert.strictEqual(date.etdfToIsoDate(example.input, true), example.output)
    }
  }),
  it("should transform etdf date to ISO datetime (@ day end)", () => {
    ...
    for (const example of examples) {
      assert.strictEqual(date.etdfToIsoDate(example.input, false), example.output)
    }
  })
})